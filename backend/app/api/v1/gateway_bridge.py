...
from app.core.security import generate_secret, hash_token, verify_secret
...
def get_current_bridge(request: Request, db: DbSession) -> GatewayBridge:
    """Authenticate a bridge by mTLS fingerprint **and** a per-bridge secret.

    F-4 (defense in depth): the edge-provided client-cert fingerprint alone is
    not enough — the bridge must also present its shared secret, which the API
    verifies (constant-time) against a stored hash. A bridge without a stored
    secret hash can never authenticate (fail-closed); it must be rotated.
    """
    settings = get_settings()
    raw = request.headers.get(settings.bridge_cert_header)
    if not raw:
    ...
    presented = request.headers.get(settings.bridge_secret_header, "")
    if not verify_secret(presented, bridge.secret_hash or ""):
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "Invalid bridge credentials")
    return bridge
...
@router.post("/bridges", response_model=GatewayBridgeCreated, status_code=status.HTTP_201_CREATED)
def register_bridge(
    ...
    # F-4: mint a per-bridge secret; store only its hash and return it once.
    secret = generate_secret()
    bridge = GatewayBridge(
        organization_id=org_id, name=body.name, cert_fingerprint=fingerprint,
        secret_hash=hash_token(secret), is_active=True,
    )
    db.add(bridge)
    db.flush()
    ...
    db.commit()
    db.refresh(bridge)
    return GatewayBridgeCreated(**GatewayBridgeOut.model_validate(bridge).model_dump(), secret=secret)
