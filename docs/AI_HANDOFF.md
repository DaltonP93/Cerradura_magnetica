...
- **Trabajo pendiente:** cola de PRs de fix — **próximo: revocación automática de tarjetas→outbox** (backend, testeable), luego Redis (rate-limit/revocación multi-worker), y **UI completa de doble aprobación** (frontend; backend confirmado).
- **Próxima tarea recomendada:** **revocación de tarjetas→outbox** en rama aislada con tests; luego Redis o UI de doble aprobación.
