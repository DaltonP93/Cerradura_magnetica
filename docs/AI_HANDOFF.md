...
- **PR #21 (F-6, abierto):** SHA `55c10d2`. `mask_card()` en los mensajes de error del importador ("already assigned"/"duplicated"); tests afirman nº en claro ausente + `*0001` presente (cubre CSV y MDB). **Probado localmente: 177 passed; `ruff` limpio.** CI: pendiente de confirmar.
- **Cola 2ª tanda de seguridad (en curso):** F-6 (#21) ✅. Próximos: F-9, F-7, F-10, F-11, F-8, F-5.
- **Trabajo pendiente:** **backlog P1 cerrado** (Redis rate-limit #17, Redis revocación #19, pip-audit #15, migraciones como job #20; TLS ya implementado/documentado). Cola de fixes de auditoría #11–#20, todos Draft, CI verde, sin fusionar. Follow-ups de **integración a `develop`**: re-encadenar migraciones apiladas (#13 y #16), reconciliar infra frontend/CI (#10 y #18), orden de apilado Redis (#17 → #19), y combinar el `command:` de compose (#11 + #20).
- **Próxima tarea recomendada:** a criterio del líder/Codex — **integración ordenada** de los PRs a `claude/develop` (requiere autorización para fusionar), o nuevos hallazgos. No quedan ítems de backlog ni follow-ups de código pendientes.
