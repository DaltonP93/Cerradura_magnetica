...
| #19 | `claude/sec-redis-revocation-bus` | `claude/sec-redis-ratelimit` (#17) | Draft, sin fusionar | **Revocación de sesiones instantánea multi-worker** (Redis Pub/Sub, opt-in). Apilado sobre #17. |
| #20 | `claude/deploy-hardening` | `claude/develop` | Draft, sin fusionar | **P1 despliegue**: migraciones como job dedicado en compose (servicio `migrate`, evita carreras multi-réplica). |
| #21 | `claude/sec-f6-importer-mask` | `claude/develop` | Draft, sin fusionar | **F-6**: enmascarar nº de tarjeta en errores del importador. |
| #22 | `claude/sec-f9-get-or-404` | `claude/develop` | Draft, sin fusionar | **F-9**: `get_or_404` fail-closed (exige `organization_id`). |
| #23 | `claude/sec-f7-metrics-token` | `claude/develop` | Draft, sin fusionar | **F-7**: `/metrics` compare en tiempo constante + aviso en prod sin token. |
| #24 | `claude/sec-f10-inbox-conflict` | `claude/develop` | Draft, sin fusionar | **F-10**: inbox — conflicto por-fila (savepoint) no rompe el lote. |
| #25 | `claude/sec-f11-open-idempotency` | `claude/develop` | Draft, sin fusionar | **F-11**: apertura remota idempotente por `Idempotency-Key` (bridge). |
| #26 | `claude/sec-f8-login-enumeration` | `claude/develop` | Draft, sin fusionar | **F-8**: login en tiempo constante (anti-enumeración de usuarios). |
| #27 | `claude/sec-f5-mfa-recovery-codes` | `claude/develop` | Draft, sin fusionar | **F-5**: códigos de recuperación MFA de un uso (evita lockout permanente). |
...
- **Fecha/hora:** 2026-09-10 ~11:45 UTC (2026-09-10 ~08:45 `America/Asuncion`).
...
- **PR #19 (revocación instantánea multi-worker, abierto):** SHA `dc10a9c`, **base #17**. Completa la 2ª mitad del ítem Redis. `revocation_bus`: publica revocaciones (session/user/org) a un canal Redis y un subscriber daemon por worker las aplica vía `manager.close_*` (thread-safe, sin re-publicar). Opt-in por `ACP_REDIS_URL`; no-op sin Redis (revalidación BD sigue de red); publish best-effort (fail-safe, el cierre local ya ocurrió). Los 5 puntos de revocación (auth/users/organizations) pasan a `revoke_*`; start/stop en el lifespan. Tests con fakeredis (dispatch por tipo, no-op sin Redis, fail-safe, serialización, end-to-end por el hilo). **Probado localmente: 191 passed (184 base #17 + 7); `ruff` limpio.** Sin migraciones. **CI verde (verificado por check runs: backend ✅, backend-postgres ✅, frontend ✅ — run 34139794854).**
- **PR #20 (despliegue — migraciones como job, abierto):** SHA `82c58f0`, base `develop`. `alembic upgrade head` deja de correr en el arranque del backend (carrera al escalar) y pasa a un servicio one-shot `migrate` en compose; el backend `depends_on: migrate → service_completed_successfully`. `docs/DEPLOYMENT.md` (sección Migraciones) actualizada + patrón init-Job para orquestadores. **Validado con `docker compose config`** (sin `up`, sin tocar entornos reales). Sin cambios de backend/tests/migraciones. **CI verde (verificado por check runs: backend ✅, backend-postgres ✅, frontend ✅ — run 34154188859; CI no ejercita compose, solo confirma que backend/frontend siguen intactos).** **Integración:** el `command:` del backend también lo toca #11 (F-2); combinación trivial (job de migración + forwarded-ips configurable). Cierra el ítem P1 "migraciones como job"; TLS ya estaba implementado (`nginx.tls.conf`) y documentado.
- **Docs canónicos sincronizados (2026-09-07):** `IMPLEMENTATION_STATUS`/`SECURITY`/`REQUIREMENTS_TRACEABILITY`/`HARDWARE_STATUS` reflejan la cola #11–#20 + sección "Camino a operativo 100%" (commit docs `bf03608`).
- **PR #21 (F-6, abierto):** SHA `55c10d2`. `mask_card()` en los mensajes de error del importador ("already assigned"/"duplicated"); tests afirman nº en claro ausente + `*0001` presente (cubre CSV y MDB). **Probado localmente: 177 passed; `ruff` limpio.**
- **PR #22 (F-9, abierto):** `get_or_404` fail-closed: cuando se pasa `org_id`, exige el atributo `organization_id`; si el modelo no lo tiene → `RuntimeError` (nunca degrada a saltarse el scope multi-tenant). `_MISSING` centinela. 4 tests (`test_get_or_404.py`). **Probado localmente; `ruff` limpio.**
- **PR #23 (F-7, abierto):** `/metrics` — compare del token con `secrets.compare_digest`; `_warn_if_metrics_unprotected(cfg)` loguea aviso al arrancar en producción si el endpoint quedó sin token (edge-restriction sigue siendo válida, por eso aviso y no fail-fast). Tests en `test_observability.py`. **Probado localmente; `ruff` limpio.**
- **PR #24 (F-10, abierto):** inbox `ingest_events` — cada `record_event` en savepoint (`db.begin_nested()`); `IntegrityError` por colisión concurrente de `event_uid` → cuenta como duplicado y continúa (no rompe el lote); el broadcast diferido se encola solo tras flush OK (rollback seguro). Test de conflicto concurrente (monkeypatch `IntegrityError` una vez). **Probado localmente; `ruff` limpio.**
- **PR #25 (F-11, abierto):** SHA `810cb37`. `enqueue_command(..., idempotency_key=None)`; `open_door` en modo bridge usa el header `Idempotency-Key` del cliente como sufijo de la clave del outbox → doble-submit colapsa en un comando (sin doble pulso); sin clave = `uuid4` (dos acciones = dos comandos). 2 tests (`test_open_idempotency.py`). **CI verde (verificado por check runs: backend ✅, backend-postgres ✅, frontend ✅ — run 34165216074).**
- **PR #26 (F-8, abierto):** SHA `eca5d99`. Login ejecuta **siempre** una verificación bcrypt: contra el hash del usuario o, si el email no existe, contra `_DUMMY_PW_HASH` fijo (se descarta) → tiempo constante, sin enumeración por timing. Se evita el cortocircuito calculando `password_ok` antes de la rama de fallo. 401 genérico intacto. 2 tests con spy (`test_login_protection.py`). Sin migraciones. **CI verde (verificado por check runs: backend ✅, backend-postgres ✅, frontend ✅ — run 34471192384).** **Residual:** 409 de unicidad global email/serial (requiere unicidad por-tenant + migración) = follow-up.
- **PR #27 (F-5, abierto):** SHA `1ef61e5`, base `develop`. Recovery codes MFA de un uso: columna JSON `users.mfa_recovery_hashes` (migración `c5d6e7f8a9b0`, padre `e0f1a2b3c4d5`, **head único**; round-trip up/down/up limpio); `services/mfa_recovery.py` (10 códigos, hash SHA-256, consumo en tiempo constante por entrada, reasigna la lista); `enable`/`recovery-codes` devuelven los códigos **una vez** (`MfaEnableResponse`); login acepta `recovery_code` cuando falla el TOTP y lo consume (auditoría `mfa_recovery_code_used`); `disable` los limpia. Los hashes nunca se serializan (fuera de `UserOut`/`/me`). 7 tests (`test_mfa.py`). **Probado localmente: 184 passed; `ruff` limpio; head único.** CI en verificación al registrar. **Reset admin auditado = follow-up documentado** (la otra mitad del "y/o" de la mitigación).
- **Cola de fixes de auditoría COMPLETA (2026-09-10):** los **11 hallazgos F-1…F-11 tienen PR** Draft aislado con tests. F-1 (#12), F-2 (#11), F-3 (#14), F-4 (#13), F-5 (#27), F-6 (#21), F-7 (#23), F-8 (#26), F-9 (#22), F-10 (#24), F-11 (#25). No quedan hallazgos de la auditoría sin PR.
- **Trabajo pendiente:** **backlogs P0/P1/P2/P3 de la auditoría cerrados en PRs** (F-1…F-11 + R-1/R-2/R-4 + migraciones-job). **Residuales documentados** (no bloqueantes): 409 de unicidad global (F-8, por-tenant), reset admin de MFA (F-5). **Pendiente real:** enforcement de flags avanzados (R-3, requiere hardware N3000/L04) e **integración ordenada a `claude/develop`** (requiere autorización para fusionar). Follow-ups de integración: re-encadenar migraciones apiladas (#13, #16, #27 cuelgan de `e0f1a2b3c4d5`), reconciliar infra frontend/CI (#10 y #18), orden de apilado Redis (#17 → #19), combinar el `command:` de compose (#11 + #20).
- **Próxima tarea recomendada:** a criterio del líder/Codex — **integración ordenada** de los 20 PRs Draft a `claude/develop` (requiere autorización para fusionar), validación de hardware (bloqueada), o nuevos hallazgos. No quedan ítems de backlog de código de la auditoría.
