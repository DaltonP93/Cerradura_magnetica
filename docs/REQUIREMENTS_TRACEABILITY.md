...
| UI-003 | UI [P] | Flags de puerta/controladora mostrados "no aplicado/experimental" + edición deshabilitada (evitar falsa seguridad) | `OPEN_PR_UNVERIFIED` (PR abierto) | `frontend/src/lib/doorFlags.ts`, `DoorsPage.tsx`, `ControllersPage.tsx` (badges ámbar, banner, controles deshabilitados con `ADVANCED_FLAGS_ENFORCED=false`) | **PR #10** | `doorFlags.test.ts`, `DoorsPage.test.tsx`, `ControllersPage.test.tsx` | Enforcement real (P0-1) o mantener deshabilitado | La UI no sugiere protección inexistente |
...
| ID | Área | Requisito | Estado | Evidencia (@`b97f5e3`) | PR/commit | Tests | Faltante | Criterio de aceptación |
|---|---|---|---|---|---|---|---|---|
| BACKUP-001 | Backup | Backup confiable (falla si `pg_dump` falla; sin dump parcial) | `OPEN_PR_UNVERIFIED` (PR abierto) | `scripts/backup_db.sh` **endurecido** (sin pipe, verifica exit de `pg_dump`, `gzip -t`, rename atómico, `umask 077`) | **PR #9** | `test_backup_restore_scripts.py` | Confirmar job CI real | `pg_dump` fallido → código≠0, sin `.gz` final |
| BACKUP-002 | Restore | Restore seguro (swap no destructivo + rollback) y **probado** | `OPEN_PR_UNVERIFIED` (PR abierto) | `scripts/restore_db.sh` **reescrito** (temp DB → validar → rename activa→recovery, temp→activa, rollback; mutex; `--drop-recovery` explícito) | **PR #9** | `test_backup_restore_scripts.py` (22 fakes) + `test_backup_restore_integration.py` (PG real en CI) | Confirmar job CI real; prueba en entorno autorizado | Restore validado; base activa nunca se pierde |
